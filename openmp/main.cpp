...
void parse_args(int argc, char* argv[], Params& p) {
    for (int k = 1; k < argc; ++k) {
        ...
        if (a == "--nx") nexti(p.Nx);
            ...
            std::cout <<
                "Usage: ./openmp_solver [--nx N] [--ny N] [--tol T] [--max-iters M]\n"
                    ...
                    const Grid& f, double dx, double dy)
{
    ...
    // Parallelize the outer loop and sum l2diff from all threads
    #pragma omp parallel for reduction(+:l2diff)
...
void print_header(const Params& p) {
    std::cout << "==== OpenMP Jacobi (parallel) ====\n"
              << "Grid: " << p.Nx << " x " << p.Ny
              << "   tol: " << p.tol
